...
class Tienda():
    ...
    def __init__(self, ventana_producto):
        ...
        self.ventana_producto.title("TIENDA DIAZ")
        self.ventana_producto.geometry("770x700")
    ...
    def Login(self):
        ...
        self.login_frame = LabelFrame(self.ventana_producto, text="Login", font=("Comic Sans", 10, "bold"), padx=20, pady=20)
        self.login_frame.pack(padx=20, pady=20)
        
        # Usuario y Contraseña
        self.label_usuario = Label(self.login_frame, text="Usuario:")
        self.label_usuario.grid(row=0, column=0, padx=5, pady=5)
        self.entry_usuario = Entry(self.login_frame)
        self.entry_usuario.grid(row=0, column=1, padx=5, pady=5)

        self.label_contrasena = Label(self.login_frame, text="Contraseña:")
        self.label_contrasena.grid(row=1, column=0, padx=5, pady=5)
        self.entry_contrasena = Entry(self.login_frame, show="*")
        self.entry_contrasena.grid(row=1, column=1, padx=5, pady=5)

        # Botón de login
        self.boton_login = Button(self.login_frame, text="Ingresar", command=self.verificar_login)
        self.boton_login.grid(row=2, columnspan=2, pady=10)
    
    def verificar_login(self):
        usuario = self.entry_usuario.get()
        contrasena = self.entry_contrasena.get()
        
        # Ejemplo simple de credenciales
        credenciales_validas = {"admin": "1234", "usuario1": "abc123"}
        
        if usuario in credenciales_validas and credenciales_validas[usuario] == contrasena:
            messagebox.showinfo("Acceso", "¡Acceso exitoso!")
            self.login_frame.pack_forget()  # Ocultar el formulario de login
            self.abrir_ventana_principal()  # Mostrar la ventana principal
        else:
            messagebox.showerror("Error", "Credenciales incorrectas")
    
    def abrir_ventana_principal(self):
       # Crear la ventana principal de la tienda aquí (por ejemplo, los menús y widgets de la tienda)
        menubar = Menu(self.ventana_producto)   
        ventana_producto = self.ventana_producto
    ...
    def widgets_crud(self):
        ...
        #Logo 
        imagen=Image.open("./TIENDA/img/app_logo_2.png")
    ...
    def widgets_informacion(self):
        ...
        # self.Label_titulo = Label(self.Label_informacion,text="Creado por She´s Tech - 2024",fg="black",font=("Comic Sans",12,"bold"))
